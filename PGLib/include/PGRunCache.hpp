...
class PGRunCache {
public:
    static inline const std::filesystem::path s_cacheFilename = L"PGPatcher_UpdateCache.bin";
    ...
    /**
     * @brief Identity of a file in the output directory, used to detect outputs that were modified or replaced
     * between runs without reading them
     */
    ...
    struct HookOutputRecord {
        HookKind kind = HookKind::ConvertToCM;
        ...
        OutputIdentity identity; /**< Size and modification time of the generated texture */
    };
...
private:
    ...
    static inline std::unordered_map<std::wstring, OutputIdentity> s_outputSnapshot; /**< lowercase rel path */
...
public:
     ...
     * Every output file written during the run must be on disk (the file saver must have been drained), because the
     * identities of the outputs are captured here.
     *
    ...
    static void recordHookRegistration(const HookKind& kind,
                                       const std::filesystem::path& texPath);
    /**
     * @brief Records an output file written for the mesh being patched. The file does not have to exist yet (mesh
     * saving is asynchronous); its identity is captured in finishRun().
     */
    static void recordOutputFile(const std::filesystem::path& relPath,
                                 uint64_t size);

    /**
     * @brief Records a texture generated by a hook patcher (global, called from the texture phase). The output must
     * already be written.
     */
